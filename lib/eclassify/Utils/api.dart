...
import 'package:flutter_sixvalley_ecommerce/eclassify/Utils/network_request_interseptor.dart';
  ...
  static Map<String, dynamic> headers() {
    if (!HiveUtils.isUserAuthenticated()) {
      if (HiveUtils.getLanguage() != null || HiveUtils.getLanguage()?['data'] != null) {
        return {"Accept": "application/json", "Content-Language": HiveUtils.getLanguage()['code'] ?? ""};
      } else {
        return {};
      }
    } else {
...
//
      ...
      var abc = ((useBaseUrl ?? true) ? Constant.baseUrl : "") + url;

      //final response = await dio.get(((useBaseUrl ?? true) ? Constant.baseUrl : "") + url, queryParameters: queryParameters, options: Options(headers: headers()));
