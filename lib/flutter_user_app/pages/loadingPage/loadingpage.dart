  ...
  navigate() async {
    if (userRequestData.isNotEmpty && userRequestData['is_completed'] == 1) {
      ...
      if (userRequestData['requestStops']['data'].isNotEmpty) {
        for (var i = 0; i < userRequestData['requestStops']['data'].length; i++) {
          addressList.add(AddressList(
              id: userRequestData['requestStops']['data'][i]['id'].toString(),
              type: 'drop',
              address: userRequestData['requestStops']['data'][i]['address'],
              latlng: LatLng(userRequestData['requestStops']['data'][i]['latitude'],
      ...
      if (userRequestData['drop_address'] != null && userRequestData['requestStops']['data'].isEmpty) {
        addressList.add(AddressList(
            id: '2',
            type: 'drop',
            pickup: false,
            address: userRequestData['drop_address'],
            latlng: LatLng(userRequestData['drop_lat'], userRequestData['drop_lng'])));
      }
      ...
      if (val == true) {
        ...
        if (userRequestData['is_rental'] == true) {
          naviagterental();
        } else if (userRequestData['is_rental'] == false && userRequestData['drop_address'] == null) {
          naviagteridewithoutdestini();
        } else {
          navigate1();
        }
  ...
  getLanguageDone() async {
    ...
    try {
      if (platform == TargetPlatform.android) {
        ...
        print("object");
        _version = await FirebaseDatabase.instance.ref().child('user_android_version').get();
      } else {
              ...
              child: Column(
                ...
                children: [
                  Container(
                    ...
                    decoration: const BoxDecoration(
                        image: DecorationImage(image: AssetImage('assets_user_app/images/logo.png'), fit: BoxFit.contain)),
                  ),
                ],
              ),
            ),
